...
echo.
echo 4. 发送系统通知
curl -X POST "http://localhost:8081/api/admin/notice?notice=欢迎参加本次抽奖活动！"
echo.

echo.
echo 5. 开始抽奖
curl -X POST "http://localhost:8081/api/draw/start?winnerCount=1&prizeName=iPhone16 Pro"
echo.

echo.
echo 6. 抽取多个获奖者
curl -X POST "http://localhost:8081/api/draw/start?winnerCount=2&prizeName=AirPods Pro"
echo.

echo.
echo 7. 手动发布公告
curl -X POST "http://localhost:8081/api/draw/announce?userName=测试用户&prizeName=特别奖励"
echo.
